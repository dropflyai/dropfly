...
export async function POST(request: NextRequest) {
  try {
    ...
    const subscriptionTier = (userData?.subscription_tier as keyof typeof STRIPE_PLANS) || 'free';
    const planLimits = STRIPE_PLANS[subscriptionTier].limits;
    ...
    if (planLimits.aiGenerations !== -1 && currentUsage >= planLimits.aiGenerations) {
      return NextResponse.json(
        {
          error: 'Limit reached',
          message: `You've reached your monthly limit of ${planLimits.aiGenerations} AI generations. Upgrade to generate more.`,
          currentUsage,
    ...
    // Build the prompt based on creator mode
