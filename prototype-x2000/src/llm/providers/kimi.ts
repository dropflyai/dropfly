 ...
 *
 * Supports Kimi 2.5, Moonshot-v1.
 * Features: Long context support (128K+), tool calling, streaming.
 */
...
/**
 * Model definitions for Kimi/Moonshot
 */
...
const DEFAULT_MODEL = 'kimi-2.5-latest';
...
export class KimiLLM extends BaseLLM {
  ...
  get capabilities(): LLMCapabilities {
    const model = KIMI_MODELS.find((m) => m.id === this.model);
    return (
      model?.capabilities || {
        streaming: true,
        toolCalling: true,
  ...
  async chat(messages: LLMMessage[], options?: ChatOptions): Promise<LLMResponse> {
    ...
    const response = await this.makeRequest<KimiResponse>(
      '/v1/chat/completions',
      body,
      options
    );
  ...
  async *stream(
    ...
    for await (const chunk of this.makeStreamingRequest(
      ...
      for (const line of lines) {
        ...
        try {
          const event: KimiStreamChunk = JSON.parse(data);
  ...
  protected formatMessages(messages: LLMMessage[]): KimiMessage[] {
  ...
  protected formatTools(tools: ToolDefinition[]): KimiTool[] {
  ...
  protected parseToolCalls(response: KimiResponse): ToolCall[] {
  ...
  protected extractContent(response: KimiResponse): string {
    return response.choices[0]?.message?.content || '';
  }

  protected extractUsage(response: KimiResponse): LLMUsage {
    return {
  ...
  private parseResponse(response: KimiResponse): LLMResponse {
...
/**
 * Get available Kimi/Moonshot models
 */
export function getKimiModels(): ModelInfo[] {
  return KIMI_MODELS;
}

/**
 * Create a Kimi LLM instance
 */
export function createKimi(config?: Partial<LLMConfig>): KimiLLM {
  return new KimiLLM({
    provider: LLMProvider.KIMI,
